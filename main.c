...
// ASCII-based symbols for LED status
#define LED_ON  "(*)"
#define LED_OFF "( )"
...
int main(void) {
    // Initialize system
    System_Init(); // Includes clock configuration
    ...
    float prev_temperature = -100.0f; // Initialize to an impossible value

    // Main loop
    while (1) {
        delay_us(100000000); // Simulate 100µs delay
